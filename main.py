...
async def lifespan(app: FastAPI):
    ...
    # Добавить колонку due_date если её нет (для существующих БД)
    try:
...
async def get_tasks(
    ...
    - search      — поиск по названию и описанию (LIKE)
    - status      — 0=активные, 1=выполненные
    - category_id — фильтр по категории
    - priority    — 1=низкий, 2=средний, 3=высокий
    - sort_by     — created_at | priority | title | status
    - sort_dir    — asc | desc
    """
    db = get_db()
    ...
    allowed_sort = {"created_at", "priority", "title", "status"}
    s_col = sort_by if sort_by in allowed_sort else "created_at"
    s_dir = "ASC" if sort_dir == "asc" else "DESC"
    query += f" ORDER BY t.{s_col} {s_dir}"
...
async def create_task(data: TaskCreate, current_user: dict = Depends(get_current_user)):
    ...
    cur = db.cursor()
    cur.execute("INSERT INTO tasks (title,description,status,priority,user_id,category_id,due_date) VALUES (?,?,?,?,?,?,?)",
                (data.title, data.description, data.status, data.priority, current_user["id"], data.category_id, data.due_date))
    db.commit()
    task = db.execute(
...
async def update_task(task_id: int, data: TaskUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if data.due_date is not None:    updates.append("due_date=?");    params.append(data.due_date)
    if updates:
